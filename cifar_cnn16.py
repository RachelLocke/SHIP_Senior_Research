...
def load_data():
    training_file = open("train_reduced16.csv")
    ...
    test_file = open("test_reduced16.csv")
...
def cnn():
    ...
    # First Conv layer
    model.add(layers.Conv2D(filters=32, kernel_size=(3,3), strides=(1,1), padding='same', activation='relu', input_shape=(16,16,3)))
...
'''
def main():
    # Load in the reduced 16x16 image cifar10 dataset
    x_train, y_train, x_test, y_test = load_data()

    # Reshape the 50,000 image row vectors into 16x16x3
    x_train = x_train.reshape((x_train.shape[0],16,16,3))#.transpose(0,2,3,1)
    #print(x_train.shape)

    x_test = x_test.reshape((x_test.shape[0],16,16,3))#.transpose(0,2,3,1)
    #print(x_test.shape)
